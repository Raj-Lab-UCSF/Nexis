function outputs = stdNDM_mouse_PFFvsGCI_Yuanxi_Numeric(varargin)
% Parameters to fit
...
normtype_ = 'log';
w_dir_ = 0; 
param_init_ = [NaN,0,0.0058,0,1e-14,0, 1e-14];
ub_ = [1,Inf,Inf,1,Inf, Inf, Inf];
lb_ = zeros(1,7);
% lb_(6) = 1e-12;
...
%         ipR.lb(4) = 1;
    ...
    objfun_handle = @(param) objfun_eNDM_general_dir_costopts_numeric(param,...
    ...
    e_num = param_num(6);
    f_num = param_num(7);
    a_num = param_num(8); % not fit
    b_num = param_num(9); % not fit
    p_num = param_num(10); % not fit
    ynum = eNDM_general_dir_numeric(x0_num,time_stamps,C,U,alpha_num,beta_num,s_num,d_num,e_num,f_num,a_num,b_num,p_num,ipR.solvetype,ipR.volcorrect);
...
%         pathology = pathology/nansum(pathology(:,1));
        ...
        ynum = eNDM_general_dir_numeric(x0_num,time_stamps,C,U,alpha_num,beta_num,s_num,a_num,b_num,p_num,ipR.solvetype,ipR.volcorrect);
