...
studylist = {'IbaHippInj','Hurtado'}; % Cell array of test datasets
wdir = 1; % Toggle directionality fitting (s) on and off
volcorrect = 1; % ***Always keep set to 1 if possible***
usedataspace = [0,1]; % Toggle data space fitting on and off
param_init = [NaN,0,1,0.5]; % Initial fmincon parameter guesses; {gamma, alpha, beta, s}
ub = [Inf,Inf,Inf,1]; % Upper bounds for fmincon
lb = zeros(1,4); % Lower bounds for fmincon
...
niters_glob = 3; % Number of bootstrapped iterations (does nothing if bootstrap flag is 0)
bootstrapping_usemedian = 1; % Use median (1) or mean (0) for calculating optimal parameters
        ...
        for m = 1:length(excltpts_costfun)
            for k = 1:length(usedataspace)
                ...
                outputs_ng_ijkm = NexIS_global('study',study_i,...
                                              ...
                                              'use_dataspace',useds_k,...
                                              'bootstrapping',bootstrapping_glob,...
                                              'bootstrapping_usemedian',bootstrapping_usemedian,...
                                              'niters',niters_glob,...
                                              'volcorrect',volcorrect,...
                                              'param_init',param_init,...
                                              'ub',ub,...
...
%
%
%% 1.4 User-specified connectome & pathology, default seeds from tau datasets
%
%
%
% Running NexIS
...
studylist = {'IbaHippInj','DS4','BoludaCBD','Hurtado'}; % Cell array of test dataset names
wdir = 1; % Toggle directionality fitting (s) on and off
volcorrect = [0,1]; % Should be able to set custom volumes now
usedataspace = [0,1]; % Should be able to set custom dataspace now
param_init = [NaN,0,1,0.5]; % Initial fmincon parameter guesses; {gamma, alpha, beta, s}
ub = [Inf,Inf,Inf,1]; % Upper bounds for fmincon
lb = zeros(1,4); % Lower bounds for fmincon
excltpts_costfun = [];
...
niters_glob = 3;
bootstrapping_usemedian = 1;
customseeds = {'none'}; % Specify a custom seed if desired

% Table output parameters
writetofile = 1; % Create .csv from MATLAB table
filename_out = 'NexIS_Wrapper_Global_1-4_user-specified'; % Name of output file
filepath_out = '~/Documents/MATLAB/Nexis_Project/Results_Files_NexISWrapper'; % Save path

% Load relevant data files for test (theoretically, could use anything)
load([matdir filesep 'Mouse_Tauopathy_Data_HigherQ.mat'],'mousedata_struct'); % CCF versions of data
load([matdir filesep 'Connectomes.mat'],'Connectomes');
load([matdir filesep 'DefaultAtlas.mat'],'DefaultAtlas');
volumes = {[],DefaultAtlas.volumes}; % specify volumes for volcorrect
...
% Run model and create output tables for each dataset, if writetofile = 1
outputs_all = struct;
numsims = length(studylist)*length(volcorrect)*length(usedataspace)*...
    length(bootstrapping_glob)*length(customseeds);
for i = 1:length(studylist)
    study_i = studylist{i};
    data_i = mousedata_struct.(study_i).data; % Extract data
    ts_i = mousedata_struct.(study_i).time_stamps; % Extract time points
    regions_i = mousedata_struct.(study_i).regions(:,2); % Extract region coregistration
    tablename = [filename_out '_' study_i]; % Create one output table per study
    sumtable_i = [];
    for j = 1:length(customseeds)
        if strcmp(customseeds{j},'none')
            seed_j = mousedata_struct.(study_i).seed; % Extract seed
        elseif strcmp(customseeds{j},'wb')
            seed_j = ones(size(data_i,1),1); % Uniform seeding
        elseif strcmp(customseeds{j},'random')
            seed_j = rand(size(data_i,1),1); % Randomized seed
        end
        for k = 1:length(volcorrect)
            volumes_k = volumes{k};
            for m = 1:length(usedataspace)
                for n = 1:length(bootstrapping_glob)
                    tablerowname = ['global, ' study_i ', '];
                    if volcorrect(k)
                        tablerowname = [tablerowname 'with volume corrs, '];
                    else
                        tablerowname = [tablerowname 'no volume corrs, '];
                    end
                    if usedataspace(m)
                        tablerowname = [tablerowname 'data space, '];
                    else
                    ...
                    tablerowname = [tablerowname 'custom seed ' customseeds{j}];
                    if bootstrapping_glob(n)
                        tablerowname = [tablerowname ', with b.s.'];
                    else
                        tablerowname = [tablerowname ', no b.s.'];
                    end
                    simno = length(bootstrapping_glob)*length(volcorrect)*...
                        length(usedataspace)*length(customseeds)*(i-1)...
                       + length(bootstrapping_glob)*length(volcorrect)*length(usedataspace)*(j-1) + ...
                       length(bootstrapping_glob)*length(usedataspace)*(k-1) ...
                       + length(bootstrapping_glob)*(m-1) + n;
                    fprintf('NexIS Wrapper Test 1.4, %d/%d\n',simno,numsims)
                    fprintf('Simulation: %s\n',tablerowname)
                    outputs_ng_ijkmn = NexIS_global('study','User_specified',...
                                                  'C',C,...
                                                  'data',data_i,...
                                                  'seed',seed_j,...
                                                  'tpts',ts_i,...
                                                  'regions',regions_i,...
                                                  'w_dir',wdir,...
                                                  'use_dataspace',usedataspace(m),...
                                                  'bootstrapping',bootstrapping_glob(n),...
                                                  'niters',niters_glob,...
                                                  'bootstrapping_usemedian',bootstrapping_usemedian,...
                                                  'volcorrect',volcorrect(k),...
                                                  'volumes',volumes_k,...
                                                  'param_init',param_init,...
                                                  'ub',ub,...
                                                  'lb',lb,...
                                                  'excltpts_costfun',excltpts_costfun);
                    fieldname_ijkmn = [study_i '_volcorrect_' num2str(volcorrect(k))...
                        '_use_ds_' num2str(usedataspace(m)) '_seed_' customseeds{j} '_bs_' ...
                        num2str(bootstrapping_glob(n))];
                    outputs_all.(fieldname_ijkmn) = outputs_ng_ijkmn;
                    sumtable_ijkmn = Output2Table(outputs_ng_ijkmn,0,'null','null'); % create table row
                    sumtable_ijkmn.Properties.RowNames{1} = tablerowname; % label table row
                    sumtable_i = [sumtable_i; sumtable_ijkmn]; % add row to table
                end
            end
        end
    end
...
%
%
%% 1.5 User-specified connectome & pathology, custom seeds
%
%
%
% Running NexIS
...
niters_glob = 3;
bootstrapping_usemedian = 1;
customseeds = {'wb','random'}; % Specify a custom seed if desired

% Table output parameters
writetofile = 1; % Create .csv from MATLAB table
filename_out = 'NexIS_Wrapper_Global_1-5_user-specified_custom_seeds'; % Name of output file
filepath_out = '~/Documents/MATLAB/Nexis_Project/Results_Files_NexISWrapper'; % Save path
        ...
        for k = 1:length(bootstrapping_sv)
            for m = 1:size(nexsvnames,2)
                ...
                outputs_sv_ijkm = NexIS_SV('study',study_i,...
                                              ...
                                              'niters_nexis_sv',niters_sv,...
                                              'bootstrapping_usemedian',bootstrapping_usemedian,...
                                              'bootstrapping_nexis_sv_usemedian',bootstrapping_usemedian);
...
%
%
%% 2.4 User-specified connectome & pathology, default seeds from tau datasets
%
%
%
% Loading previously run NexIS_global struct. Not required to do this to run
...
% dataset
rng(0); clear; clc;
filename_out = 'NexIS_Wrapper_SV_2-4_user-specified'; % Name of output file
filename_in_glob = 'NexIS_Wrapper_Global_1-4_user-specified'; % NexIS:global input file
filepath_in = '~/Documents/MATLAB/Nexis_Project/Results_Files_NexISWrapper'; % Load path, sims
filepath_out = filepath_in; % Save path
writetofile = 1;
...
for i = 1:length(mdlinstances)
    % Grab each previously run NexIS:global instance
    mdlinstance_i = mdlinstances{i};
    tablename_i = [filename_out '_Model_Instance_' num2str(i)]; % Create one output table per output instance
    outputs_i = nexglob_outputs.outputs_all.(mdlinstance_i);
    outputs_i_inputstruct = outputs_i.nexis_global.Full;
    % Pull inputs from NexIS:global instance
    study_i = outputs_i_inputstruct.init.study;
    ...
    wdir_i = outputs_i_inputstruct.init.w_dir;
    volcorrect_i = outputs_i_inputstruct.init.volcorrect;
    volumes_i = outputs_i_inputstruct.init.volumes;
    usedataspace_i = outputs_i_inputstruct.init.use_dataspace;
    regions_i = outputs_i_inputstruct.init.regions;
    excltpts_costfun_i = outputs_i_inputstruct.init.excltpts_costfun;
    % bootstrapping_usemedian_i = outputs_i_inputstruct.init.bootstrapping_usemedian;
    bootstrapping_nexis_sv_i = outputs_i_inputstruct.init.bootstrapping;
    bootstrapping_usemedian_i = 1; % wasn't saved before; should be fixed for future runs
    % Output table row definitions
    sumtable_i = [];
    tablerowname = ['SV, ' study_i '_Data'];
    for j = 1:size(nexsvnames,2)
        simno = size(nexsvnames,2)*(i-1) + j;
        fieldname_ij = sprintf('Model_Instance_%d_%d',i,j);
        ...
        outputs_nsv_ij = NexIS_SV('outputs_nexisglobal',outputs_i,...                                  
                                  ...
                                  'datatype_nexis_sv',nexsvnames{1,j},...
                                  'datalist_nexis_sv',nexsvnames{2,j},...                
                                  'bootstrapping_nexis_sv',bootstrapping_nexis_sv_i,...              
                                  'bounds_type_nexis_sv',bounds_type_nexis_sv,...
                                  'niters_nexis_sv',niters_sv,...
                                  'bootstrapping_nexis_sv_usemedian',bootstrapping_usemedian_i);
        outputs_all.(fieldname_ij) = outputs_nsv_ij;
        sumtable_ij = Output2Table(outputs_nsv_ij,0,'null','null'); % create table row
        sumtable_ij.Properties.RowNames{1} = ['Global' tablerowname(3:end)]; % label table row
        sumtable_ij.Properties.RowNames{2} = [tablerowname ', Factor ' num2str(j)]; % label table row
...
%
%
%% 2.5 User-specified connectome & pathology, custom seeds
%
%
%
% Loading previously run NexIS_global struct. Not required to do this to run
...
% dataset
rng(0); clear; clc;
filename_out = 'NexIS_Wrapper_SV_2-5_user-specified'; % Name of output file
filename_in_glob = 'NexIS_Wrapper_Global_1-5_user-specified_custom_seeds'; % NexIS:global input file
filepath_in = '~/Documents/MATLAB/Nexis_Project/Results_Files_NexISWrapper'; % Load path, sims
filepath_out = filepath_in; % Save path
writetofile = 1;
...
% Table output parameters
writetofile = 1; % Create .csv from MATLAB table
filename_out = 'NexIS_Wrapper_SV_2-6_PCATest'; % Name of output file
filepath_out = '~/Documents/MATLAB/Nexis_Project/Results_Files_NexISWrapper'; % Save path
