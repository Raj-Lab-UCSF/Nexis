function outputs = stdNDM_mouse_PFFvsGCI_Yuanxi_individual(varargin)

% Yuanxi's Revision:
% Study_ = 'PFFvsGCI', this script is for studying individual mouse.

% I added some outputs.
%

% Parameters to fit
% param(1:n_mice) = seed rescale factor
% param(n_mice+1) = alpha
% param(n_mice+2) = beta
% param(n_mice+3) = s
...
% Here, length(GCI mice) = 5, length(PFF mice) = 7

% GCI Mice, 1~5 seed factor, 6 alpha, 7 beta, 8 s
param_init_ = [NaN(1,5),0,1,1];
ub_ = [ones(1,5),Inf,Inf,1];
lb_ = zeros(1,8);
lb_(6) = -Inf;


%% This part is for testing PFF.
% % PFF Mice, 1~7 seed factor, 8 alpha, 9 beta, 10 s
...
elseif strcmp(ipR.study,'PFFvsGCI')
    load([cd filesep 'PFF GCI\DataInput\GCI_PFF_Data.mat' ],...
        'GCI_PFF_Pathology_Data','GCI_PFF_Seed_Data','tpts');
...
if ~logical(ipR.bootstrapping)
    ...
    time_stamps = tpts.GCI_Individual;
%     time_stamps = [3,3];
    pathology = normalizer(GCI_PFF_Pathology_Data.GCI_Individual,'log');
...
% 
%     RowLoc = [];
%     for i = 1:length(pathology_topbottom)
%         [RowLocTemp,~] = find(pathology == pathology_topbottom(i));
...
%         ipR.param_init(end) = 1;
%         ipR.ub(end) = 1;
%         ipR.lb(end) = 1;

        ipR.param_init(end) = 0; % There's a possible local optimum. Try different initials. 
        % I tested different initials and found the PFF was
        % retrograde-dominated but GCI was anterograde-dominated.
        ipR.ub(end) = 1;
        ipR.lb(end) = 0;
    end
