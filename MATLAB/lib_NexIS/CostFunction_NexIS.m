...
function f = CostFunction_NexIS(study_,U_,params_,...
...
if ismember(study_,taustudies)
    ...
    predicted = NexIS_fun(study_,U_,params_,solvetype_,volcorrect_,matdir_);
end
...
if any(isnan(pathology))
    ...
    for i = 1:length(ts)
        ...
        timeinds_i(isnan(pathology_i)) = [];
        pathology_i(isnan(pathology_i)) = [];
        predicted_i(isnan(pathology_i)) = [];
