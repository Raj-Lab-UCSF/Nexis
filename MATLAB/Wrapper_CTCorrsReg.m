...
%     B_matrix = B_matrix(2:end,:);
    ...
    B_upper_min = B_mean_min + 3*B_std_min; B_lower_min = B_mean_min - 3*B_std_min;
    ...
    B_upper_1se = B_mean_1se + 3*B_std_1se; B_lower_1se = B_mean_1se - 3*B_std_1se;
    ...
    B_upper_0 = B_mean_0 + 3*B_std_0; B_lower_0 = B_mean_0 - 3*B_std_0;
...
%     g = 1:length(classkey_tasic);
%     figure('Position',[0 0 1500 400]); hold on;
%     b = boxplot(B_matrix.',g,'Colors',cmap,'Symbol','');
%     set(b,{'linew'},{2})
%     plot([0,length(classkey_tasic)+1],[0,0],'k--','LineWidth',2);
%     set(gca,'XTick',1:length(subclasses),'XTickLabel',subclasses,'XTickLabelRotation',45);
% %     set(gca,'YTick',[-1 -0.5 0 0.5 1])
%     ylim([1.1*min(B_matrix(:)),1.1*max(B_matrix(:))])
%     ylabel('Ridge Regression Coefficients');
%     xlabel('');
%     set(gca,'TickLength',[0 0])
%     title(['Cell Type Ridge Coefficients, ' datsetlabels{i}],'FontSize',28);
%     set(gca, 'FontSize', 20, 'FontName', 'Times');
end
if plotting && savenclose
    cd /Users/justintorok/Documents/MATLAB/Nexis/Figures/;
    print(sprintf('Bootstrap_Lcurves_%s',kfold),'-dtiffn','-r300'); close;
    cd /Users/justintorok/Documents/MATLAB/Nexis/;
end
%% 
