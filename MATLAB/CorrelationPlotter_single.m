...
if isempty(sv_types)
    if logical(wdir)       
        ...
        'Units','inch','Position',[0 0 5*num_plots 5.3]);
    ...
    if logical(wdir)
        ...
        fig = figure('DefaultAxesPosition', [0.1, 0.1, 0.8, 0.8],...
        'Units','inch','Position',[0 0 5*num_plots 5.9]);
...
for j = 1:num_plots
    ...
    if logical(wdir)
        scatter(predicted(:,j),data(:,j),75,shape,'MarkerFaceColor',color,...
            'MarkerEdgeColor',color,'LineWidth',1)
    else
        scatter(predicted(:,j),data(:,j),75,shape,'MarkerFaceColor','w',...
            'MarkerEdgeColor',color,'LineWidth',1)
    end
    box on; hold on;
    plot_max_data_j = max(data(:,j));
    plot_max_pred_j = max(predicted(:,j));
    p = polyfit(predicted(:,j),data(:,j),1);
    lsx = linspace(0,1.1*plot_max_pred_j);
    lsy = polyval(p,lsx);
    plot(lsx,lsy,'--','LineWidth',3);
    h1 = lsline;
    h1.Color = 'k'; h1.LineWidth = 3; h1.LineStyle = '--';
    legend(h1,sprintf('R = %.2f',corr(data(:,j),predicted(:,j),...
        'rows','complete')),'Location','southeast')
    xticks([plot_max_pred_j/2,plot_max_pred_j]);
    xtickformat('%.2f');
    xlim([0 plot_max_pred_j*1.1]);
    yticks([plot_max_data_j/2,plot_max_data_j]);
    ytickformat('%.2f');
    ylim([0 plot_max_data_j*1.1]);
    text(-0.02*plot_max_pred_j*1.1,-0.075*plot_max_data_j*1.1,'0','FontSize',20,'FontName','Times')
    text(-0.07*plot_max_pred_j*1.1,0,'0','FontSize',20,'FontName','Times')
    if j == 1
