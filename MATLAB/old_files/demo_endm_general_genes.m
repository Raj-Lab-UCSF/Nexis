...
% seed rescale value
init_guess_params(1) = 0.0405;%(nansum(pathology(:,1)))/nnz(seed_location);
lb(1) = 0.0405;
ub(1) = 0.0405;

% alpha
init_guess_params(2) = 0.5254;
lb(2) = 0.4729;
ub(2) = 0.5779;

% beta
init_guess_params(3) = 0.6614;
lb(3) = 0.5953; % beta must be strictly non-negative
ub(3) = 0.7275;
...
lb((n_types+4):(2*n_types+3)) = -Inf;
ub((n_types+4):(2*n_types+3)) = Inf;

% p
init_guess_params((2*n_types+4):(3*n_types+3)) = 0;
lb((2*n_types+4):(3*n_types+3)) = -Inf;
ub((2*n_types+4):(3*n_types+3)) = Inf;
...
for i = 1:length(type_inds)
    ...
    output(i+1,:)={classkey{type_inds(i)}, param_num(1), param_num(2), param_num(3), param_num(3 + i),...
