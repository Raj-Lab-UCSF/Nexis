function outputs_all = stdNDM_mouse_PFFvsGCI_TestGenes_Yuanxi(varargin)
% Yuanxi's Revision:
% Study_ = 'PFFvsGCI', this script is for testing genes.
% didn't use Nexis model - not fit the parameter 'b' 

% 'BrainRegionReorderMat' in the file 'GCI_PFF_Data.mat' was used to
% reorder the gene expression array. (translate Raj_ROIs into Peng_ROIs).

% Gene_expression_diag*C → gene affect on the indegree;  
% C*Gene_expression_diag → gene affect on the outdegree;
% I added some outputs.
% Remember to adjust the initial value of the 's' parameter. Otherwise it
% will lead to local optimum.
...
elseif strcmp(ipR.study,'PFFvsGCI')
    load([cd filesep 'PFF GCI\DataInput\GCI_PFF_Data.mat' ],...
        'GCI_PFF_Pathology_Data','GCI_PFF_Seed_Data','tpts', 'regvgene_mean', 'BrainRegionReorderMat','gene_names_trans');
...
% for gene_num = 1:5
    ...
    try
        ...
        for i = 1:row_reorder
            for j = 1:col_reorder
                if isnan(BrainRegionReorderMat(i,j))
                    ...
                    SNCA_expression_Mat(i,j) = Gene_expression_raw(BrainRegionReorderMat(i,j));
                end
            end
        end
        Gene_expression = mean(SNCA_expression_Mat,2,'omitnan');
        ...
        C = C*Gene_expression_diag;  % Gene_expression_diag*C → gene affect on the indegree;  C*Gene_expression_diag → gene affect on the outdegree;
        % C = .*C;
        ...
        if ~logical(ipR.bootstrapping)
            ...
            time_stamps = tpts.PFF_Average; % if for GCI, use 'GCI_Average'
            pathology = normalizer(GCI_PFF_Pathology_Data.PFF_Average,'log'); % if for GCI, use 'PFF_Average_New'. 'PFF_Average' included the outlier point which we removed.
            ...
            if ~logical(ipR.w_dir)
                ...
                %         ipR.ub(4) = 1;
                %         ipR.lb(4) = 1;
                ipR.param_init(4) = 0.05;  % Initial value: GCI 0.05; PFF 0.95;
                ipR.ub(4) = 1;
                ipR.lb(4) = 0;
            end
