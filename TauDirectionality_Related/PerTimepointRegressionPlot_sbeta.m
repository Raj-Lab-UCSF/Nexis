function PerTimepointRegressionPlot_sbeta(outstruct,use_s,savenclose_,figdir_)
...
s_betamat = NaN(length(studynames),length(tptnames)); % 4 models
tptmat = s_betamat;
for i = 1:size(s_betamat,1)
    if use_s
        ...
        for k = 1:length(tptnames)
            ...
            s_betamat(i,k) = resstruct_tpt.param_fit(4);
        end
    else
        resstruct = outstruct.(studynames{i}).('fit_s');
        ylab = '\beta';
        for k = 1:length(tptnames)
            resstruct_tpt = resstruct.(tptnames{k}).nexis_global.Full;
            tptmat(i,k) = resstruct_tpt.time_stamps;
            s_betamat(i,k) = resstruct_tpt.param_fit(3);
        end
        ...
        %     s_betamat(i,k) = corr_ret - corr_ant;
        % end
    end
end
...
for i = 1:length(studynames)
    s = scatter(tptmat(i,:),s_betamat(i,:),75,shapes{i},...
        'MarkerFaceColor',cmap(i,:),'MarkerEdgeColor',cmap(i,:),...
        'MarkerFaceAlpha',0.3);
    plothands = [plothands,s];
end
lm = fitlm(tptmat(:),s_betamat(:));
...
if ~use_s
    yplotmax = max(s_betamat(:)); yplotmin = min(s_betamat(:));
    ...
    loc = 'northeast';    
    text(0.75,0.5,Rptext,'FontSize',20,'FontName','Times','Units','normalized');
else
...
if savenclose_
    if use_s
        print([figdir_ filesep 'pertimepointregplot_s'],'-dtiffn','-r300'); close;
    else
        print([figdir_ filesep 'pertimepointregplot_beta'],'-dtiffn','-r300'); close;
    end
end
